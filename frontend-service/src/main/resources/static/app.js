...
const uploadBase = (window.DOCUDEEP_UPLOAD_BASE || '').replace(/\/$/, '');
const viewBase = (window.DOCUDEEP_VIEW_BASE || '').replace(/\/$/, '');
...
const refreshCasesButton = document.getElementById('refresh-cases');
...
async function submitCase() {
  ...
  try {
    ...
    const caseResponse = await fetch(`${uploadBase}/cases`, {
      method: 'POST',
...
async function uploadDocument(caseId, plan, file) {
  try {
    ...
    const completeResponse = await fetch(`${uploadBase}/cases/${caseId}/documents/${plan.documentId}/complete`, {
      method: 'POST'
    });
...
async function refreshCaseList() {
  ...
  try {
    const response = await fetch(`${viewBase}/cases`);
    if (!response.ok) {
    ...
    if (!payload.length) {
      ...
      item.textContent = 'Aucun dossier n\'est disponible.';
      caseList.appendChild(item);
      return;
    }

    payload.forEach(caseSummary => {
      const item = document.createElement('li');
      item.classList.add('case-item');
      const title = document.createElement('div');
      title.classList.add('case-id');
      title.textContent = caseSummary.caseId;
      const count = document.createElement('span');
      count.classList.add('badge');
      count.textContent = `${caseSummary.documents.length} document(s)`;
      item.appendChild(title);
      item.appendChild(count);

      if (caseSummary.documents.length) {
        const docList = document.createElement('ul');
        docList.classList.add('document-list');
        caseSummary.documents.forEach(doc => {
          const docItem = document.createElement('li');
          const link = document.createElement('a');
          link.href = `${viewBase}/cases/${caseSummary.caseId}/documents/${doc.id}`;
          link.textContent = `${TYPE_LABELS[doc.documentType] ?? doc.documentType} – ${doc.filename}`;
          link.target = '_blank';
    ...
    const item = document.createElement('li');
    item.classList.add('muted');
    item.textContent = `Impossible de récupérer les dossiers : ${error.message}`;
    caseList.appendChild(item);
  }
}
...
if (resetButton) {
  resetButton.addEventListener('click', async () => {
    try {
      const resp = await fetch(`${uploadBase}/dev/storage/reset`, { method: 'POST' });
      if (resp.ok) {
...
if (refreshCasesButton) {
  refreshCasesButton.addEventListener('click', refreshCaseList);
}
